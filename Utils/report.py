...
def generate_pdf_report(class_label, confidence, image, segmented_img):
    ...
    report_id = f"TN-{now.strftime('%Y%m%d%H%M%S')}-{uuid.uuid4().hex[:6].upper()}"
    model_version = "TumorNet v2.1.0"
    ...
    out_name = f"TumorNet_Report_{now.strftime('%Y%m%d_%H%M%S')}.pdf"
    ...
    logo_path = None
    for candidate in ("logo.png", "tumornet_logo.png", "logo.jpg"):
    ...
    flow.append(Spacer(1, 24))
    flow.append(Paragraph("TumorNet AI System — Advanced Brain Tumor Detection Platform", subtitle_style))
    flow.append(Spacer(1, 8))
